...
class Character:
    ...
    def check_on_ground(self, map_instance, TILE_SIZE):
        ...
        foot_y = (self.y + self.height) // TILE_SIZE
        ...
        if foot_y < len(map_instance.map_data) and foot_x < len(map_instance.map_data[0]):
            ...
            # タイルが1の場合の処理
            if tile == 1 and self.y + self.height <= (foot_y * TILE_SIZE + ground_mid_height):
                self.on_ground = True
                return foot_y * TILE_SIZE + ground_mid_height - self.height
            # タイルが2の場合の処理
            elif tile == 2:
                self.on_ground = True
                return foot_y * TILE_SIZE - self.height
        # 接地していない場合
        self.on_ground = False
        return self.y
